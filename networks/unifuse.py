...
class UniFuse(nn.Module):
    ...
    def forward(self, input_equi_image, input_cube_image):
        ...
        if self.num_layers < 18:
            ...
            input_equi_image = input_equi_image.unsqueeze(0)  # 假设 input_data 是你的输入张量
        ...
        input_cube_image = input_cube_image.unsqueeze(0)  # 假设 input_data 是你的输入张量
        print(input_cube_image.shape)

        cube_inputs = torch.cat(torch.split(input_cube_image, self.cube_h, dim=-1), dim=0)
        print(cube_inputs.shape)
